      ...
      <div class="add-product">
        <div class="add-product__col add-product__col_1">
          <div class="white-item">
            <ul class="list-check">
              <li class="list-check__item">
                <label class="check-table">
                  ...
                  <span class="label-text">Опубликован</span>
                </label>
              </li>
            </ul>
            <div class="box-form-row">
              <div class="box-form-row__col">
                <label class="box-field__label" style="white-space: nowrap">
                  Код товара
                  <img
                    class="refresh"
                    :style="{transform: 'rotate(' + refreshButtonRotate + 'deg)'}"
                    src="@/assets/img/refresh.png"
                    alt="refresh"
                    @click="getNextCode"
                    @mouseenter="refreshButtonRotate += 360" />
                </label>
              ...
              <div class="box-form-row__col" style="width: 100%">
                <colorSelect :def="form.color_ids" :items="colors" v-model="form.color_ids" />
              </div>
            </div>
            <div class="box-form-row">
              <div class="box-form-row__col">
            ...
            <categorySelect :def="form.category_id" :items="productCategories" v-model="form.category_id"/>
            <subCategorySelect :def="form.subcategory_id" :items="productSubCategories" :parent="form.category_id" v-model="form.subcategory_id"/>
            <brandSelect :def="form.brand_ids" :items="productBrands" v-model="form.brand_ids"/>
            <div class="box-form-row">
              <div class="box-form-row__col">
            ...
            <ul class="list-check">
              ...
              <li class="list-check__item">
                <label class="check-table">
                  <input type="radio" v-model="form.sex" value="unisex" >
                  <span class="check-table__icon"></span>
            ...
            <ul class="list-check">
              <li class="list-check__item">
                <label class="check-table">
                  <input type="checkbox" v-model="form.seasons.winter" checked >
                  <span class="check-table__icon"></span>
              ...
              <li class="list-check__item">
                <label class="check-table">
                  <input type="checkbox" v-model="form.seasons.spring" >
                  <span class="check-table__icon"></span>
              ...
              <li class="list-check__item">
                <label class="check-table">
                  <input type="checkbox" v-model="form.seasons.summer" >
                  <span class="check-table__icon"></span>
              ...
              <li class="list-check__item">
                <label class="check-table">
                  <input type="checkbox" v-model="form.seasons.autumn" >
                  <span class="check-table__icon"></span>
            ...
            <div class="box-form-row">
              <div class="box-form-row__col">
                <label class="box-field__label"><span style="white-space: nowrap">Стоимость доставки</span>,<br>руб</label>
                <div class="box-field__input">
                  <input v-model.number="form.deliveryPrice" type="text" class="form-control">
                </div>
              </div>
              <div class="box-form-row__col">
                <label class="box-field__label">Наценка,<br>руб</label>
                <div class="box-field__input">
                  <input v-model.number="form.overprice" type="text" class="form-control">
                </div>
              </div>
            </div>
            <ul class="tabs">
        ...
        <div class="add-product__col add-product__col_3">
          ...
          <div class="add-product__col_tablet">
            <div class="white-item">
              ...
              <ul class="menu-tabs">
                <li v-for="(item, index) in form.images" :class="{'active' : activeImage === index}" :key="index" @click.prevent="activeImage = index">
                  № {{index + 1}}
              ...
              <div v-for="(item, index) in form.images" v-show="activeImage === index" :key="index">
                <categorySelect :def="form.images[index].category_id" :items="productCategories" v-model="form.images[index].category_id"/>
                <subCategorySimple :def="form.images[index].subcategory_id" :items="productSubCategories" :parent="form.images[index].category_id" v-model="form.images[index].subcategory_id"/>
                <productSelect :def="form.images[index]._id" :category="form.images[index].category_id" :subCategory="form.images[index].subcategory_id" v-model="form.images[index].item_id" @product="productCategory = $event"/>
                <qualitySelect :def="form.images[index]._id" v-model="form.images[index].quality" :product="productCategory" />
              </div>
            </div>
          </div>
          <div class="add-product__col_tablet">
            <div class="white-item">
              ...
              <div v-for="(item, index) in form.youMightLike" v-show="activeLike === index" :key="index">
                <categorySelect :def="form.youMightLike[index].category_id" :items="productCategories" v-model="form.youMightLike[index].category_id"/>
                <subCategorySimple :def="form.youMightLike[index].subcategory_id" :items="productSubCategories" :parent="form.youMightLike[index].category_id" v-model="form.youMightLike[index].subcategory_id"/>
                <productSelect :def="form.youMightLike[index]._id" :category="form.youMightLike[index].category_id" :subCategory="form.youMightLike[index].subcategory_id" v-model="form.youMightLike[index].item_id" @product="productCategory2 = $event"/>
                <qualitySelect :def="form.youMightLike[index].quality" v-model="form.youMightLike[index].quality" :product="productCategory2" />
              </div>
            </div>
          </div>
          <div class="add-product__col_tablet">
            <button @click.prevent="updateProductIn()" class="btn-big-black">Обновить товар</button>
          </div>
        </div>
      </div>
    </form>
...
export default {
  ...
  data () {
    return {
      productCategory: {},
      productCategory2: {},
      refreshButtonRotate: 0,
      renderedTime: Date.now(),
      form: {
        published: false,
        code: '',
        name: '',
  ...
  watch: {
    'form.name' () {
      if (this.renderedTime + 2000 < Date.now()) { this.form.alt = translit().transform(this.form.name, '-') + '-' + this.form.code }
    },
    'form.code' () {
      if (this.renderedTime + 2000 < Date.now()) { this.form.alt = translit().transform(this.form.name, '-') + '-' + this.form.code }
    }
  ...
  created () {
    ...
    this.getProductById(this.$route.params.id)
      ...
      .then(() => {
        this.goToExistingQuality()
        this.sizeCustom()
      })
  },
  methods: {
    ...mapActions('products', ['getCategories', 'getSubCategories', 'getBrands', 'getColors', 'updateProduct', 'getProductById']),
    ...
    goToExistingQuality () {
      this.form.qualities.middle.site ? this.selectedQuality = 'middle' : this.form.qualities.premium.site ? this.selectedQuality = 'premium' : this.selectedQuality = 'high'
    }
  }
}
</script>
